...
def plot_train_test(X_train, y_train, X_val, y_val):
...
def display_fit(x_train, y_train, b, w, color='k'):
...
def display_fit_error(x_train, y_train, b, w):
    fig, ax = display_fit(x_train, y_train, b, w)
...
def range_plot(x_train, y_train, b, w, bs, ws, all_losses):
...
def cross_section_fixed_b(x_train, y_train, b, w, bs, ws, all_losses):
    b_minimum, w_minimum = fit_model(x_train, y_train)
...
def cross_section_fixed_w(x_train, y_train, b, w, bs, ws, all_losses):
    b_minimum, w_minimum = fit_model(x_train, y_train)
...
def visualize_gradient(b, w, bs, ws, all_losses):
    b_range = bs[0, :]
    w_range = ws[:, 0]
...
def annotated_delta_sensitivity(b, w, bs, ws, all_losses):
    b_range = bs[0, :]
    w_range = ws[:, 0]
...
def regression_line_after_single_descent(x_train, y_train, b, w):
    ...
    fig, ax = display_fit(x_train, y_train, b_initial, w_initial)
...
def learning_rate_plot(b, w, bs, ws, all_losses, manual_grad_b, manual_grad_w, lr):
    b_range = bs[0, :]
    w_range = ws[:, 0]
...
def loss_surface_good_bad(x_train, y_train, b, w, bad_bs, bad_ws, bad_x_train):
...
def cost_function_good_bad(x_train, y_train, b, w, bad_bs, bad_ws, bad_x_train):
    bad_b_range = bad_bs[0, :]
    bad_w_range = bad_ws[:, 0]
...
def loss_surface_good_bad_scaled(x_train, y_train, scaled_bs, scaled_ws, bad_x_train, scaled_x_train):
    # So we recompute the surface for X_TRAIN using the new ranges
